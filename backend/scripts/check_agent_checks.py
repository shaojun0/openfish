...
def check_validator() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ai-checks-") as tmp:
        root = Path(tmp)
        (root / "src").mkdir()
        # ``__init__.py`` makes this a *regular* package.  Without it the
        # fixture's ``src`` is only a namespace portion, and a same-named
        # regular package installed in site-packages wins the import scan
        # (``tarsafe`` ships one into site-packages), so the baseline check
        # would die on ``No module named 'src.calc'`` for a reason that has
...
def check_curator_loop() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ai-loop-") as tmp:
        ...
        (seed / "src").mkdir()
        # A regular package (see the note in ``check_validator``): a bare
        # namespace ``src`` loses the import scan to the ``src`` package that
        # ``tarsafe`` installs into site-packages, and the baseline check would
        # fail for a reason unrelated to the curator.
