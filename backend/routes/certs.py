...
directory listing.  ``scripts/check_auth_guards.py`` records the endpoint as
public on purpose; the permission system is not the right control here because
the client needs the CA *before* it can be authenticated.
"""
