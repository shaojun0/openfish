...
#: can see the boundary stated positively as well.  This is documentation plus a
#: regression guard (``scripts/check_git_boundary.py`` asserts none of them is in
#: an allowlist that a gate executor would pass).
...
def sandbox_env(
    ...
    *source* defaults to ``os.environ``; *extra* is merged **after** filtering
    ...
    environment (the project's own ``make gates``) must opt out in
