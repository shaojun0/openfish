...
Two integration details the gates require (both in S0.md):

1. ``auth.permissions`` must re-export these eight **constants** (not only the
   strings).  ``check_permission_catalog.py`` resolves a guard's argument with
   ``getattr(auth.permissions, <name>)`` and
   ``check_permission_labels.py`` records the bare name from the AST, so
   ``require_permission(REPO_READ)`` only passes those gates when
   ``REPO_READ`` is importable from ``auth.permissions``.
2. The guards must actually exist before the codes are merged into
   ``BUILTIN``: ``check_permission_catalog.py`` fails a declared point that no
   guard checks ("no dead points").  Merge this module in the same change that
   lands ``routes/repos.py`` / ``routes/findings.py`` / ``routes/agent_tasks.py``,
   or the catalogue gate will (correctly) report eight unused points.
...
``services.authz._AUTHENTICATED_SEED`` and classified in
``scripts/check_permission_catalog.py``; S0.md lists the intended classification
