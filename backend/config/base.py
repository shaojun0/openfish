...
* **The variable name is derived, never re-typed.**  :meth:`EnvSettings.env_name`
  ...
  ``env_prefix`` (or an explicit alias).  Error messages, log lines and
  ``scripts/check_env_ownership.py`` all ask the model instead of hard-coding a
  second copy of the name, so a renamed field cannot silently stop being read.
"""
