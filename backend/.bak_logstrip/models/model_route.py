...
* :data:`PROVIDERS` — the **wire format** (``openai`` / ``anthropic``).  It
  decides the request shape and the default endpoint path.  Only formats that
  need different requests are listed: a MinerU-style document parser answers the
  OpenAI format, so it is an ``openai`` route whose ``kind`` is ``ocr`` rather
  than a protocol of its own.
...
#: ``mineru`` was once a third one; a MinerU endpoint speaks the OpenAI format, so
#: it is registered as ``openai`` with ``kind: ocr`` and
#: :mod:`models.model_route_migrate` rewrites the rows that predate this.
