...
:func:`undocumented_endpoints`; adding a route therefore means adding its
documentation under ``backend/openapi/``.
"""
...
TAGS: list[dict[str, str]] = [
    ...
    {"name": "Node builds", "description": "Prebuilt Node.js mirror consumed by `nvm`, `fnm` and `node-gyp`."},
...
def undocumented_endpoints(app: Flask) -> list[str]:
    ...
    An empty list means the spec covers the whole contract.  Adding a route
    without documenting it must fail rather than silently vanish from
    ``/openapi.json``.
    """
    missing: list[str] = []
    for rule in app.url_map.iter_rules():
