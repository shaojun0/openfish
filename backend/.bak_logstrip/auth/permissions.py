...
# and re-exported here because a guard argument is resolved by name against this
# module — `getattr(auth.permissions, <bare name>)` — and the declaration is
# recorded by its bare AST name.  A route that writes
# `require_permission(REPO_READ)` therefore needs `REPO_READ` importable from
# here, not merely present in `BUILTIN`.
