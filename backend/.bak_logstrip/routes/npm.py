...
line, or the guard is applied after registration and never runs.  The guard must
be in effect before the route registers.
...
publish is answered ``401`` and never by the binder — installs.  That ordering
and the error envelope are part of the contract, so changing them needs care.
"""
