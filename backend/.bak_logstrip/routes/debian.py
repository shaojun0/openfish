...
def _artifact_response(text: str, filename: str, sha256: str) -> Response:
    """A generated text artifact, offered as a download.

    The filename is server-generated (``openfish-debian-snapshot-<stamp>.txt``),
    but it still goes through the shared escaping helper rather than an f-string:
    one way to write a ``Content-Disposition`` in this codebase, so the next
    caller cannot pick the unescaped one.
    """
    return Response(
        text,
